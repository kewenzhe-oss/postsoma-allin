...
class PokerTable:
    ...
    def is_round_complete(self) -> bool:
        ...
        for p in active_players:
            max_amount = max(max_amount, p.bet_in_round)
            
        # 如果未全押玩家数量 <= 1，只需要检查该玩家是否已经跟注到最高下注额
        # 在1v1游戏中，如果有一人全押，另一人跟注后，剩余的街道就不需要再行动了
        if len(non_all_in_players) <= 1:
            if non_all_in_players:
                if non_all_in_players[0].bet_in_round < max_amount:
                    return False
            return True
        ...
        return len(bet_amounts) <= 1  # 如果所有非全押玩家下注相等，回合结束
